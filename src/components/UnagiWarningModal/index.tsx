...
export default function UnagiWarningModal({ isOpen, onConfirm }: { isOpen: boolean; onConfirm: () => void }) {
    ...
    <Modal isOpen={isOpen} onDismiss={handleDismiss} maxHeight={90}>
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          <AutoRow gap="6px">
            <StyledWarningIcon />
            <Text>Notice for trading Unagi</Text>
          </AutoRow>
          <>
            <Text>
              To trade UNAGI, you must click on the settings icon and{' '}
              <strong>set your slippage tolerance to 12%+</strong>
            </Text>
            <Text>This is because Unagi taxes a 10% fee on each transaction.</Text>
            <Text>• 7% fee = redistributed to all existing holders</Text>
            <Text>• 3% fee = used to add liquidity</Text>
          </>
          <RowBetween>
            <div>
